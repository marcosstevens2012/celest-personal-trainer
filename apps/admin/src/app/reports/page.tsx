  ...
  const growthStats =
    currentMonth && previousMonth
      ? {
          ...
          payments: calculateGrowth(
            currentMonth.payments,
            previousMonth.payments
          ),
        }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              Analiza el rendimiento de tu negocio de entrenamiento personal
            </p>
          </div>
          <select
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {currentMonth?.students || 0}
                </p>
              </div>
              <div
                className={`flex items-center ${
                  growthStats.students >= 0 ? "text-green-600" : "text-red-600"
                }`}
              >
                {growthStats.students >= 0 ? (
                  <TrendingUp className="h-4 w-4 mr-1" />
                ) : (
                  <TrendingDown className="h-4 w-4 mr-1" />
                )}
                <span className="text-sm font-medium">
                  {Math.abs(growthStats.students).toFixed(1)}%
                </span>
              </div>
            </div>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              ...
              <div
                className={`flex items-center ${
                  growthStats.revenue >= 0 ? "text-green-600" : "text-red-600"
                }`}
              >
                {growthStats.revenue >= 0 ? (
                  <TrendingUp className="h-4 w-4 mr-1" />
                ) : (
                  <TrendingDown className="h-4 w-4 mr-1" />
                )}
                <span className="text-sm font-medium">
                  {Math.abs(growthStats.revenue).toFixed(1)}%
                </span>
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-2">vs mes anterior</p>
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {currentMonth?.plans || 0}
                </p>
              </div>
              <div
                className={`flex items-center ${
                  growthStats.plans >= 0 ? "text-green-600" : "text-red-600"
                }`}
              >
                {growthStats.plans >= 0 ? (
                  <TrendingUp className="h-4 w-4 mr-1" />
                ...
                <span className="text-sm font-medium">
                  {Math.abs(growthStats.plans).toFixed(1)}%
                </span>
              </div>
            </div>
            <p className="text-xs text-gray-500 mt-2">vs mes anterior</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatCurrency(
                    currentMonth
                      ? currentMonth.revenue / currentMonth.students
                      : 0
                  )}
                </p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500">
                    {((paymentStats.paid / paymentStats.total) * 100).toFixed(
                      1
                    )}
                    %
                  </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500">
                    {(
                      (paymentStats.overdue / paymentStats.total) *
                      100
                    ).toFixed(1)}
                    %
                  </div>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">
                  Estudiantes que se dieron de baja:
                </span>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="text-sm text-gray-600">
                <strong className="text-gray-900">Insight:</strong> Tu tasa de
                retención está por encima del promedio de la industria (85%).
              </div>
            </div>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="p-6 text-center">
            <div className="text-3xl font-bold text-green-600 mb-2">
              {Math.round(
                monthlyStats.reduce((sum, month) => sum + month.students, 0) /
                  monthlyStats.length
              )}
            </div>
            <div className="text-sm text-gray-600">
              Promedio Estudiantes Activos
            </div>
          </Card>

          <Card className="p-6 text-center">
            <div className="text-3xl font-bold text-purple-600 mb-2">
              {Math.round((paymentStats.paid / paymentStats.total) * 100)}%
            </div>
